...
import { Form, Input, Button, message, Spin, Typography, Card, Space, Tooltip } from 'antd'; // Thêm Space cho icon mạng xã hội
...
import {
  ...
  InstagramOutlined,
  YoutubeOutlined,
  TwitterOutlined, // Ant Design dùng TwitterOutlined cho X (trước đây là Twitter)
  LinkedinOutlined,
} from '@ant-design/icons';

// Import hình ảnh (giả định bạn có ảnh trong public/images)
...
export default function ContactPage() {
  ...
  return (
    <div className="container lg:p-12 mx-auto p-4 md:p-8">
      ...
      {/* Phần dưới: Google Map và Thông tin Liên hệ từ Config */}
      ...
      {isLoadingConfig ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          ...
          <div className="h-80 md:h-96 w-full rounded-lg overflow-hidden shadow-lg">
            {config.googlemap ? ( // Sử dụng trường googlemap
              <iframe
                    ...
                    {config.zalo && (
                      <Tooltip title="Zalo">
                        <a href={`https://zalo.me/${config.zalo}`} target="_blank" rel="noopener noreferrer">
                          {/* Zalo không có icon trong Ant Design, có thể dùng icon Font Awesome hoặc custom SVG */}
                    ...
                    {config.tiktok && (
                      <Tooltip title="TikTok">
                        <a href={config.tiktok} target="_blank" rel="noopener noreferrer">
                          {/* TikTok cũng không có icon Ant Design, dùng Font Awesome hoặc custom SVG */}

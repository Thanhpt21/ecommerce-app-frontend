...
export default function WishlistPage() {
  // Chỉ sử dụng namespace 'wishlist' và 'products' (nếu cần cho 'remove_from_wishlist')
  const { t } = useTranslation(['wishlist', 'products']);
  const { locale } = useLocaleContext();
  const { items: wishlistItems, removeItem } = useWishlist();

  const handleRemoveFromWishlist = (productId: number, productTitle: string) => {
    removeItem(productId);
    // Sử dụng namespace 'wishlist' cho thông báo
    message.success(t('removed_from_wishlist_page', { productTitle, ns: 'wishlist' }));
  };
  ...
  return (
    <div className="container lg:p-12 mx-auto p-4 md:p-8">
      <div className="mb-4">
        <Breadcrumb>
          <Breadcrumb.Item>
            {/* Sử dụng home từ common, nên vẫn phải giữ common hoặc thêm home vào wishlist.json */}
            <Link href={`/${locale}`}>
              {t('home', { ns: 'common' })} {/* Giữ lại ns: 'common' nếu không chuyển 'home' sang wishlist.json */}
            </Link>
          </Breadcrumb.Item>
          <Breadcrumb.Item>{t('title', { ns: 'wishlist' })}</Breadcrumb.Item> {/* Đổi từ 'wishlist' sang 'title' trong namespace wishlist */}
        </Breadcrumb>
      </div>
      ...
      {wishlistItems.length === 0 ? (
        <Empty
          description={t('empty_wishlist_description', { ns: 'wishlist' })} 
          image={Empty.PRESENTED_IMAGE_SIMPLE}
        >
          <Link href={`/${locale}/san-pham`}>
            <Button type="primary">{t('browse_products', { ns: 'wishlist' })}</Button> {/* Đổi từ 'common' sang 'wishlist' */}
          </Link>
        </Empty>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {wishlistItems.map((product) => (
            ...
            >
              <Tooltip title={t('remove_from_wishlist', { ns: 'wishlist' })}> {/* Giữ lại ns: 'products' */}
                <Button
                  type="text"
                  danger
              ...
              <Link href={`/${locale}/san-pham/${product.slug}`} className="flex flex-col items-start">
              ...
              {/* Tùy chọn: Nút thêm vào giỏ hàng (có thể cần logic thêm) */}
              {/* <Button
                type="primary"
                icon={<ShoppingCart className="w-4 h-4 mr-2" />}
                className="mt-4 w-full"
              >
                {t('add_to_cart', { ns: 'products' })}
              </Button> */}
            </Card>
          ))}
        </div>
